...
class ActNet(nn.Module):
    ...
    def forward(self, x_, is_train):
        ...
        #(batch, city_size, embedding_size) -> (batch, embedding_size)
        ...
        mask = torch.zeros(self.batch, self.city_size,dtype=torch.bool).to(DEVICE)
        pro = torch.FloatTensor(self.batch, self.city_size-1).to(DEVICE)  # 每个点被选取时的选取概率,将其连乘可得到选取整个路径的概率
        distance = torch.zeros(self.batch).to(DEVICE)  # 总距离
        seq = torch.zeros(self.batch, self.city_size-1).to(DEVICE)  # 选择的路径序列
        for i in range(self.city_size-1):
            #退出条件
            # if i == self.city_size - 1:
            #     break
            mask_temp = torch.zeros(self.batch, self.city_size,dtype=torch.bool).to(DEVICE)
            mask_temp[torch.arange(self.batch), idx_last] = 1 
            mask = mask | mask_temp
            # for j in range(self.batch):
            #     mask[j, idx_last[j]] = 1
            mask_ = mask.unsqueeze(1)#(batch, 1, city_size)
            mask_ = mask_.expand(self.batch, self.M, self.city_size)
            mask_ = mask_.unsqueeze(2)#(batch, M, 1, city_size)
            ...
            k = k.contiguous().view(self.batch, self.city_size, self.M, self.dk)
            v = v.contiguous().view(self.batch, self.city_size, self.M, self.dk)
            q = q.permute(0, 2, 1, 3)#(batch, M, 1, dk)
            k = k.permute(0, 2, 3, 1)#(batch, M, dk, city_size)
            v = v.permute(0, 2, 1, 3)#(batch, M, city_size, dk)
            qk = torch.matmul(q, k) / (self.dk ** 0.5)#(batch, M, 1, city_size)
            #mask
            qk.masked_fill_(mask_, -float('inf'))
            qk = F.softmax(qk, dim=-1)#(batch, M, 1, city_size)
            ...
            k = self.wk5(x)#(batch, city_size, embedding_size)
            k = k.permute(0, 2, 1)#(batch, embedding_size, city_size)
            qk = torch.matmul(q, k) / (self.dk ** 0.5)#(batch, 1, city_size)
            qk = qk.sum(dim=1)#(batch, city_size)
            qk = torch.tanh(qk)*C#TODO: c待查
            qk.masked_fill_(mask, -float('inf'))
            p = F.softmax(qk, dim=-1)#(batch, city_size)

            if is_train:
                idx = torch.multinomial(p, 1)[:, 0]#(batch, 1) ->[:,0]->(batch)
            else:
                idx = torch.argmax(p, dim=1)#(batch, city_size) ->(batch)
    ...
    def attention(self, x, wq, wk, wv, w, add_residual):
        ...
        k = wk(x)
        v = wv(x)
        q = q.contiguous().view(self.batch, self.city_size, self.M, self.dk)
        k = k.contiguous().view(self.batch, self.city_size, self.M, self.dk)
        v = v.contiguous().view(self.batch, self.city_size, self.M, self.dk)
        q = q.permute(0, 2, 1, 3)#(batch, M, city_size, dk)
        k = k.permute(0, 2, 3, 1)#(batch, M, dk, city_size)
        v = v.permute(0, 2, 1, 3)#(batch, M, city_size, dk)
        qk = torch.matmul(q, k) / (self.dk ** 0.5)#(batch, M, city_size, city_size)
        qk = F.softmax(qk, dim=-1)
        z = torch.matmul(qk, v)#(batch, M, city_size, dk)
        z = z.permute(0, 2, 1, 3)#(batch, city_size, M, dk)
        z = z.contiguous().view(self.batch, self.city_size, self.embedding_size)
        z = w(z)
        if add_residual:
            z = z + x
        return z
